...
%sigMarkerPos=[+0.01,-0.05];

% if isfield(dataStruct,'addParams')
%     xlim(dataStruct.addParams.componentInfluencePlotyLim);
% end
...
%if nargin < 3
    ...
    sigMarkers = varargin{2};
    sigMarkerPos = varargin{3};
...
dataList = varargin{1};
handles.bar = barh(dataList); % standard implementation of bar fn
hold on
hBar = handles.bar;
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        %handleerrorbar = herrorbar(values(xOrder,col), mean(x,1), lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k');
        sigmarkers = sigMarkers(xOrder,col);
        handleerrorbar = herrorbarstars(values(xOrder,col), mean(x,1), lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k', sigmarkers, sigMarkerPos);
        
        
        hErrorbar(col) = handleerrorbar(1);
