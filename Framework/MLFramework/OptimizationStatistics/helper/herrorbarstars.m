function hh = herrorbarstars(x, y, l, u, symbol, significanceMarker, sigMarkerDeltaPos)
...
for ii=1:numel(significanceMarker)
    sigText = '';
    dx = sigMarkerDeltaPos.DeltaSigMarkersX;
    dy = sigMarkerDeltaPos.DeltaSigMarkersY;
    
    if significanceMarker(ii) > 0
         sigText = '(!)';
         text(xr(ii)+dx(1),ytop(ii)+dy,sigText);
    end
    if significanceMarker(ii) < 0
         sigText = '(!)';
         text(xl(ii)+dx(2),ytop(ii)+dy,sigText);
    end    
