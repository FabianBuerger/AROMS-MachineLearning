  ...
  %  welcome;
    ...
    switch method
        ...
        case 'EigValue95'
            ...
            % Evaluate residuals
            no_dims = 0;
            while no_dims < size(X, 2) - 1 && lambda(no_dims + 1) > 0.05
                no_dims = no_dims + 1;
            end      
        case 'EigValueMin10PercentVariance'
            % Perform PCA
            [mappedX, mapping] = pca(X, size(X, 2));
            lambda = mapping.lambda ./ sum(mapping.lambda);
            ...
            % Evaluate residuals
            no_dims = 0;
            while no_dims < size(X, 2) - 1 && lambda(no_dims + 1) > 0.10
                no_dims = no_dims + 1;
            end
        case 'PCAEigValSumCriterion'
            % Perform PCA
            [mappedX, mapping] = pca(X, size(X, 2));
            lambda = mapping.lambda ./ sum(mapping.lambda);
            % Evaluate residuals
            no_dims = 1;
            lambdaCumSum = cumsum(lambda);
            %figure(); plot(lambdaCumSum)
            tresh = 0.8;
            indicesGreaterThan = find(lambdaCumSum > tresh);
            if numel(indicesGreaterThan) > 0
                no_dims = indicesGreaterThan(1);
            end
            no_dims = max(1,min(no_dims,size(X, 2)));        
        case 'MLE'
            % Set neighborhood range to search in
            k1 = 6;
            k2 = 12;
