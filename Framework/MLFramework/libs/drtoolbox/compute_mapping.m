...
%     end
    ...
    switch type
        ...
        case {'paramtSNE', 'paramt-SNE'}
            ...
            test_X = [];
            test_labels = [];
%            layers = [500 500 2000 no_dims]; %originally proposed paramtertrization, 2 in original on last layer!!
            layers = [200 200 1000 no_dims]; %faster
...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
        ...
        case {'KernelPCAGauss'}
            % Apply kernel PCA with polynomial kernel
			[mappedA, mapping] = kernel_pca(A, no_dims,'gauss');
            mapping.name = 'KernelPCA';

        case {'KernelPCAPoly'}
            % Apply kernel PCA with polynomial kernel
			[mappedA, mapping] = kernel_pca(A, no_dims,'poly');
            mapping.name = 'KernelPCA';            
		...
		case {'KernelLDAGauss'}
			% Apply generalized LDA with Gaussian kernel
             mappedA = gda(A(:,2:end), uint8(A(:,1)), no_dims, 'gauss');
            mapping.name = 'KernelLDA';
            
		case {'KernelLDAPoly'}
			% Apply generalized LDA with poly kernel
             mappedA = gda(A(:,2:end), uint8(A(:,1)), no_dims, 'poly');
            mapping.name = 'KernelLDA';
