function [M, L, Y, C] = lmnn(X, labels, kNeighbors)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if ~exist('kNeighbors', 'var')
        no_targets = 3;         % number of target neighbors
    else
        no_targets = kNeighbors;         % number of target neighbors
    end

    timeLimitSeconds = 10; % limit time
    ...
    timerStart = tic();
    timeLimitReached = 0;
    while ~timeLimitReached && ((prev_C - C > tol || iter < min_iter) && iter < max_iter)
        ...
        timeLimitReached = toc(timerStart) > timeLimitSeconds;
    end
