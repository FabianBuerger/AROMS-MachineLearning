...
% - add information about instance dependencies
%
% -------
...
function demoMLFrameworkSVMTuning_InstanceDependencies()
...
% show dependency of instances by using training dataset 4 times
dataSetTrain = dataSetInstanceSubset(dataSetTrain,1:30); % only 30 instances
nInstances = numel(dataSetTrain.targetClasses);
crossValidationInstanceDependencyInformation = [1:nInstances, 1:nInstances, 1:nInstances, 1:nInstances ]';
dataSetTrain = appendDatasets(dataSetTrain,dataSetTrain);
dataSetTrain = appendDatasets(dataSetTrain,dataSetTrain);
...
% boost calculation with parallel processing
...
analysisParams.crossValidationInstanceDependencyInformation = crossValidationInstanceDependencyInformation;
...
analysisParams.optimizationComponentsFeatureTransform = {'none'};
